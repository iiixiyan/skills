...
description: 北京单场让球胜平负(让球SPF)专用预测 — 先预测实际比分，再推算让球SPF，首/次选独立计算不强制不同
---
...
0. **先预测实际比分，再推算SPF** — 先计算预期净胜球→映射到实际比分→再用比分+盘口算出SPF。禁止直接用GD±阈值判断SPF。
1. **每场比赛必须从多个数据源同时获取数据** — 不依赖单一数据源，并行拉取所有可用数据源后进行交叉验证和综合分析
2. **根据比赛特征动态选择预测模型** — 分析联赛类型、排名差、赛季阶段、球队状态等特征后，选择最适合的模型，而非固定用一个模型
3. **预测目标是让球后的胜平负结果** — 即官方SP值对应的让球SPF，不是普通胜平负
4. **严禁从让球数反推预测** — 禁止"因为让球X所以主队难赢"这类逻辑，预测必须基于球队自身实力分析
5. **所有数据必须实时获取** — 禁止使用训练数据截止前的内部知识，必须通过API或爬虫获取最新数据
6. **让球数仅供最终验证参考** — 在独立分析完成后，用让球数来检验预测是否合理，而非用让球数来推导
...
#### 案例1：排名高球队vs保级队
热刺(#6,60分) vs 利兹联(#16,38分) — 实际0-0平
- pts差22分但热刺无欲无求，利兹联保级战意高
- **修正**: GD -0.50（保级客队战意补偿）

#### 案例2：双保级对话
巴列卡(#12,42分) vs 赫罗纳(#7,57分) — 实际1-1平
- 两队均有保级压力，不应偏向任何一方
- **修正**: GD ×0.3（压缩差距至平局区）

#### 案例3：英冠保级客队爆发
米尔沃(#14,58分) vs 赫尔城(#20,48分) — 实际0-2
- 排名低的保级客队完胜中游主队
- **修正**: GD -0.60（保级客队反常爆发）

#### 案例4：高排名球队vs盘口过深
本菲卡(#2,72分) vs 布拉加(#4,59分) -1 — 实际2-2
- 盘口-1已反映实力差，H2H不应叠加
- **修正**: GD +0.70（消除盘口+战意的双重惩罚）
...
```python
H2H = {
    ("那不勒","博洛尼"): -0.20,  # 博洛尼亚近年交手不落下风
}
# 本菲卡vs布拉加：盘口-1已足够，不再叠加H2H
```

### 首/次选规则

**预测流程：先预测实际比分 → 再推算让球胜平负**

首/次选两个比分独立计算各自SPF，**不强制次选与首选不同**。

| 步骤 | 说明 |
|:----:|:------|
| ① 预测比分 | 根据球队实力/战意/5月系数 → 输出两个差异化的实际比分 |
| ② 计算SPF | 让球方(主队)调整后 vs 对方：调整后＞对方→让球胜，＝→让球平，＜→让球负 |
| ③ 输出 | 每场三行：让球描述 → 比分(主选+次选) → SPF(主选+次选不同时才显示) |

```python
# 计算方式
def calc_spf(home_goals, away_goals, handicap):
    """从实际比分推算让球SPF"""
    adjusted = home_goals + handicap  # 让球方(主队)调整后进球
...
```
{№}│{主队}({盘口}) vs {客队}│{置信度}
   让球: {让球描述}（如"贝蒂斯让1球"或"光州F受让2球"或"平手"）
   比分: 主选 {比分1}  次选 {比分2}
   让球后: 主选 {SPF1}  [次选 {SPF2}]  ← 次选仅当SPF1≠SPF2时显示
```
